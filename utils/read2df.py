...
def unify_dfs(dfs, symbols, period):
    ...
    for index, row in df.iterrows():
        if index <= period:
            ...
            reg = LinearRegression().fit(np.array(df_tmp['close1']).reshape(-1, 1), df_tmp['close0'])
            resid = row['close0']-reg.predict(np.array(row['close1']).reshape(-1, 1))
            spread.append(resid[0])
    ...
    for index, row in df.iterrows():
        if index <= period:
            ...
            df_tmp = df.iloc[index-period:index]
            std = df_tmp['spread'].std()
            zscore = (row['spread']-df_tmp['spread'].mean())/std if std!=0 else 0
